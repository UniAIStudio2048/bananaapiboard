export function createCheckoutController({ api, onChange = () => {}, onPaid = () => {}, now = Date.now, schedule = setTimeout, cancel = clearTimeout }) {
  ...
  async function open(payload, existingId) {
    generation++; stopRequest(); input = payload; requestKey = key(); delivered = false; failures = 0
    ...
    try {
      if (existingId) {
        ...
        state.attempt = order.attempts?.find(a => a.status === 'pending_review') || order.attempts?.at(-1) || null
