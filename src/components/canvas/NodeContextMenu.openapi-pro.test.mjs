...
test('NodeContextMenu does not wait for asset polling after OpenAPI Pro face submit', () => {
  assert.match(source, /if \(isOpenApiPro\) \{[\s\S]*showToast\('Seedance 角色已提交审核', 'success'\)[\s\S]*canvasStore\.addNode\({[\s\S]*assetUri:\s*savedAssetUrl[\s\S]*status:\s*initialStatus[\s\S]*url:\s*savedAssetUrl[\s\S]*return[\s\S]*\}[\s\S]*const \{ promise \} = pollAssetStatus/)
})
