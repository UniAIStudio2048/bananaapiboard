...
// 大画布投影刷新时保留 Vue Flow 内部的实时拖拽坐标，拖拽结束后再统一写回 store。
// 坐标 Map 本身不做响应式，避免每个 pointermove 都重建 Vue Flow 节点列表。
const activeDragPositions = new Map()
const activeDragNodeIds = ref(new Set())

function updateActiveDragPositions(event) {
  const draggedNodes = event?.nodes?.length ? event.nodes : [event?.node]
  const nodeIds = new Set(activeDragNodeIds.value)
  let idsChanged = false
  for (const node of draggedNodes) {
    const x = Number(node?.position?.x)
    const y = Number(node?.position?.y)
    if (node?.id && Number.isFinite(x) && Number.isFinite(y)) {
      activeDragPositions.set(node.id, { x, y })
  ...
  if (idsChanged) activeDragNodeIds.value = nodeIds
}

function clearActiveDragPositions() {
  activeDragPositions.clear()
  if (activeDragNodeIds.value.size > 0) activeDragNodeIds.value = new Set()
}
...
const renderProjectionActiveIds = computed(() => {
  ...
  for (const nodeId of activeDragNodeIds.value) ids.add(nodeId)
  return ids
})
...
const renderProjection = computed(() => {
  ...
  const projection = projectCanvasRenderState({
    ...
    positionOverrides: activeDragPositions,
...
onNodeDragStart((event) => {
  clearActiveDragPositions()
  updateActiveDragPositions(event)
