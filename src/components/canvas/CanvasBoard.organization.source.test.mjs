...
assert.match(source, /const\s+groupDragStartPositions\s*=\s*new\s+Map\(\)/)
assert.match(source, /onNodeDragStart\([\s\S]*?groupDragStartPositions\.set/)
assert.match(source, /onNodeDragStop\([\s\S]*?emit\('organization-mutation-end'\)[\s\S]*?\n\}\)/)
assert.match(source, /onNodeDragStop\([\s\S]*?groupDragStartPositions\.delete\(node\.id\)/)
