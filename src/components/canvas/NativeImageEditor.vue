...
const liquifyBrushSize = ref(80)
const liquifyPressure = ref(50)
...
const mixerColorChannels = [
  {
    ...
    width: 34,
    track: {
      hue: 'linear-gradient(90deg, #ec4899, #ef4444, #f97316)',
      saturation: 'linear-gradient(90deg, #9ca3af, #ef4444)',
      lightness: 'linear-gradient(90deg, #450a0a, #ef4444, #fecaca)'
    }
  },
  {
    id: 'orange',
    ...
    width: 28,
    track: {
      hue: 'linear-gradient(90deg, #ef4444, #f97316, #facc15)',
      saturation: 'linear-gradient(90deg, #a3a3a3, #f97316)',
      lightness: 'linear-gradient(90deg, #431407, #f97316, #fed7aa)'
    }
  },
  {
    id: 'yellow',
    ...
    width: 28,
    track: {
      hue: 'linear-gradient(90deg, #f97316, #eab308, #84cc16)',
      saturation: 'linear-gradient(90deg, #a3a3a3, #eab308)',
      lightness: 'linear-gradient(90deg, #422006, #eab308, #fef3c7)'
    }
  },
  {
    id: 'green',
    ...
    width: 40,
    track: {
      hue: 'linear-gradient(90deg, #84cc16, #22c55e, #14b8a6)',
      saturation: 'linear-gradient(90deg, #9ca3af, #22c55e)',
      lightness: 'linear-gradient(90deg, #052e16, #22c55e, #bbf7d0)'
    }
  },
  {
    id: 'aqua',
    ...
    width: 36,
    track: {
      hue: 'linear-gradient(90deg, #22c55e, #2dd4bf, #38bdf8)',
      saturation: 'linear-gradient(90deg, #94a3b8, #2dd4bf)',
      lightness: 'linear-gradient(90deg, #042f2e, #2dd4bf, #ccfbf1)'
    }
  },
  {
    id: 'blue',
    ...
    width: 42,
    track: {
      hue: 'linear-gradient(90deg, #38bdf8, #3b82f6, #8b5cf6)',
      saturation: 'linear-gradient(90deg, #94a3b8, #3b82f6)',
      lightness: 'linear-gradient(90deg, #172554, #3b82f6, #bfdbfe)'
    }
  },
  {
    id: 'purple',
    ...
    width: 34,
    track: {
      hue: 'linear-gradient(90deg, #3b82f6, #8b5cf6, #d946ef)',
      saturation: 'linear-gradient(90deg, #a1a1aa, #8b5cf6)',
      lightness: 'linear-gradient(90deg, #2e1065, #8b5cf6, #ddd6fe)'
    }
  },
  {
    id: 'magenta',
    ...
    width: 34,
    track: {
      hue: 'linear-gradient(90deg, #8b5cf6, #ec4899, #ef4444)',
      saturation: 'linear-gradient(90deg, #a1a1aa, #ec4899)',
      lightness: 'linear-gradient(90deg, #500724, #ec4899, #fbcfe8)'
    }
  }
]

function buildDefaultHslMixer() {
  return mixerColorChannels.reduce((result, channel) => {
    result[channel.id] = { hue: 0, saturation: 0, lightness: 0 }
    return result
  }, {})
}

function normalizeHslMixer(value = {}) {
  const next = buildDefaultHslMixer()
  mixerColorChannels.forEach(channel => {
    const saved = value?.[channel.id] || {}
    mixerAdjustmentKeys.forEach(key => {
      next[channel.id][key] = Number(saved[key] || 0)
    })
  })
  return next
}

const hslMixer = ref(normalizeHslMixer())
...
const mixerTabs = [
  ...
  { id: 'lightness', label: '明亮度' },
  { id: 'liquify', label: '液化' }
]

const activeMixerRange = computed(() => {
  if (activeMixerTab.value === 'hue') {
    return { min: -60, max: 60, step: 1, unit: '' }
  }
  return { min: -100, max: 100, step: 1, unit: '' }
})

const isLiquifyActive = computed(() => currentMode.value === 'filter' && activeMixerTab.value === 'liquify')
...
function getChannelWeight(hue, channel) {
  const distance = getHueDistance(hue, channel.hue)
  if (distance >= channel.width) return 0
  return 1 - distance / channel.width
}
...
function hasHslMixerAdjustments() {
  return mixerColorChannels.some(channel => {
    const values = hslMixer.value[channel.id]
    return mixerAdjustmentKeys.some(key => Number(values?.[key] || 0) !== 0)
  })
}
...
function applyColorMixerToCanvas() {
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    const nextRgb = hslToRgb(
      hsl.h + hueOffset / totalWeight,
      hsl.s + (saturationOffset / totalWeight) / 100,
      hsl.l + (lightnessOffset / totalWeight) / 100
    )
    data[i] = nextRgb.r
    data[i + 1] = nextRgb.g
    data[i + 2] = nextRgb.b
...
function getSnapshotSource(entry) {
  const source = entry?.snapshotUrl || entry?.imageData || null
  return getProxiedImageUrl(source) || source
}

function createHistoryEntryFromCurrentCanvas(options = {}) {
  if (!options.snapshotUrl && !mainCanvasRef.value) return null

  const imageData = options.snapshotUrl || mainCanvasRef.value.toDataURL('image/png')
  if (!imageData) return null

  return {
    snapshotUrl: imageData,
    filters: { ...filters.value },
    hslMixer: normalizeHslMixer(hslMixer.value),
...
function seedInitialHistoryFromSource() {
  history.value = []
  historyIndex.value = -1

  pushHistoryEntry(createHistoryEntryFromCurrentCanvas({
    snapshotUrl: getProxiedImageUrl(props.imageUrl) || props.imageUrl
  }))
}
...
function fitDisplaySize(width, height) {
  ...
  const mixerWidth = 340
  const reserveMixerWidth = currentMode.value === 'filter' || props.initialTool === 'filter'
  const availableMaxWidth = reserveMixerWidth
    ? Math.max(360, maxWidth - mixerWidth)
    : maxWidth

  let nextWidth = width
  let nextHeight = height

  if (nextWidth > availableMaxWidth) {
    nextHeight = nextHeight * (availableMaxWidth / nextWidth)
    nextWidth = availableMaxWidth
  }
...
async function restoreFromCachedState(state) {
  ...
  try {
    ...
    hslMixer.value = normalizeHslMixer(state.hslMixer)
    ...
    brushHardness.value = state.brushHardness ?? state.maskBrushHardness ?? 100
    brushOpacity.value = state.brushOpacity ?? state.maskBrushOpacity ?? 100
...
function renderMaskPreview() {
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    if (brightness > 0) {
      // 白色/灰色区域 -> 按蒙版强度显示半透明红色
      previewData.data[i] = 255     // R
      previewData.data[i + 1] = 0   // G
      previewData.data[i + 2] = 0   // B
      previewData.data[i + 3] = Math.min(180, Math.max(20, Math.round(brightness * 0.7)))
    } else {
      // 黑色区域 -> 透明
...
function activateMode(mode) {
  ...
  if (isLiquifying.value) {
    endLiquify()
  }

  // 离开蒙版模式时清除预览（但保留蒙版数据）
  if ((currentMode.value === 'mask' || currentMode.value === 'spot-heal') && mode !== 'mask' && mode !== 'spot-heal') {
    clearOverlay()
  }
...
function startDraw(e) {
  if (currentMode.value !== 'draw' && currentMode.value !== 'mask' && currentMode.value !== 'spot-heal') return
  ...
  if (currentMode.value === 'mask' || currentMode.value === 'spot-heal') {
    paintMaskBrushPoint(x, y, getCurrentBrushSettings())
    renderMaskPreview()
  } else if (currentMode.value === 'draw') {
    paintDrawBrushPoint(x, y, getCurrentBrushSettings())
  }
}
...
function draw(e) {
  ...
  if (currentMode.value === 'mask' || currentMode.value === 'spot-heal') {
    drawMaskBrushStroke(lastX.value, lastY.value, x, y)
    // 更新蒙版预览
    renderMaskPreview()
  } else {
...
function drawMaskBrushStroke(fromX, fromY, toX, toY) {
  const ctx = maskCtx.value
  if (!ctx) return

  paintBrushPath(fromX, fromY, toX, toY, getCurrentBrushSettings(), paintMaskBrushPoint)
}

function getCurrentBrushSettings() {
  return normalizeMaskBrushSettings({
...
function drawBrushStroke(fromX, fromY, toX, toY) {
  ...
  const settings = getCurrentBrushSettings()
  if (settings.hardness >= 100) {
    ctx.save()
    ctx.globalAlpha = getMaskBrushAlpha(settings.opacity)
    ctx.beginPath()
    ctx.moveTo(fromX, fromY)
    ctx.lineTo(toX, toY)
    ctx.strokeStyle = brushColor.value
    ctx.lineWidth = scaleUiSize(settings.size)
    ctx.lineCap = 'round'
    ctx.lineJoin = 'round'
    ctx.stroke()
    ctx.restore()
  ...
  paintBrushPath(fromX, fromY, toX, toY, settings, paintDrawBrushPoint)
}

function paintBrushPath(fromX, fromY, toX, toY, settings, painter) {
  const radius = scaleUiSize(settings.size) / 2
...
function paintDrawBrushPoint(x, y, settings) {
  const ctx = mainCtx.value
  if (!ctx) return

  const scaledSize = scaleUiSize(settings.size)
  const radius = scaledSize / 2
  const alpha = getMaskBrushAlpha(settings.opacity)
  const featherRadius = scaleUiSize(getMaskBrushFeatherRadius(settings.size, settings.hardness))
  const innerRadius = Math.max(0, radius - featherRadius)
  const color = parseCanvasColor(brushColor.value)
...
function paintMaskBrushPoint(x, y, settings) {
  const ctx = maskCtx.value
  if (!ctx) return

  const scaledSize = scaleUiSize(settings.size)
  const radius = scaledSize / 2
  const intensity = Math.round(255 * getMaskBrushAlpha(settings.opacity))
  const featherRadius = scaleUiSize(getMaskBrushFeatherRadius(settings.size, settings.hardness))
  const innerRadius = Math.max(0, radius - featherRadius)
...
function resetFilters() {
  ...
  hslMixer.value = normalizeHslMixer()
  commitFilters()
}

function resetMixerControl(channelId, key = activeMixerTab.value) {
...
function getLiquifyPressureValue() {
  return clamp(Number(liquifyPressure.value || 0) / 100, 0, 1)
}

function ensureLiquifyResetSnapshot() {
  if (!mainCanvasRef.value || liquifyResetSnapshot.value) return
  liquifyResetSnapshot.value = mainCanvasRef.value.toDataURL('image/png')
}

function setMixerTab(tabId) {
  if (isLiquifying.value) {
    endLiquify()
  }

  const wasLiquifyActive = activeMixerTab.value === 'liquify'
  activeMixerTab.value = tabId
  clearOverlay()
...
function startLiquify(e) {
  if (!isLiquifyActive.value || !mainCtx.value) return

  ensureLiquifyResetSnapshot()
  const { x, y } = getCanvasCoords(e)
  isLiquifying.value = true
  liquifyChanged.value = false
...
function resetLiquify() {
  if (!liquifyResetSnapshot.value || !mainCtx.value) return
  ...
  img.src = liquifyResetSnapshot.value
}
...
function handleMouseDown(e) {
  if (isLiquifyActive.value) {
    startLiquify(e)
  } else if (currentMode.value === 'draw' || currentMode.value === 'mask' || currentMode.value === 'spot-heal') {
    startDraw(e)
...
function handleWheel(e) {
  ...
  } else if (isLiquifyActive.value) {
    const delta = e.deltaY > 0 ? -5 : 5
    liquifyBrushSize.value = Math.max(5, Math.min(300, liquifyBrushSize.value + delta))
  }
}
...
<template>
  <div class="native-editor-wrapper" ref="containerRef" :style="{ width: width + 'px' }">
    ...
    <div class="editor-toolbar">
      ...
      <div class="toolbar-section">
        ...
        <div class="toolbar-group">
          ...
          <button
            class="tool-btn"
            :class="{ active: currentMode === 'spot-heal' }"
            @click="activateMode('spot-heal')"
            title="污点修复"
          >
            <span class="tool-icon">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14.7 6.3a4 4 0 010 5.7l-6.4 6.4a4 4 0 01-5.7-5.7L9 6.3a4 4 0 015.7 0z"/><path d="M6.5 14.5l3 3"/><path d="M16 4l1-2"/><path d="M19 7l2-1"/><path d="M18 11l2 1"/></svg>
            </span>
            <span class="tool-label">污点修复</span>
          </button>
        </div>
        ...
        <div class="toolbar-group">
          ...
          >
            <span class="tool-icon">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="13.5" cy="6.5" r="2.5"/><circle cx="17.5" cy="13" r="2.5"/><circle cx="9" cy="16.5" r="2.5"/><path d="M7.5 3.5A10 10 0 1020.5 18c.7-1.2-.2-2.7-1.6-2.7h-1.2"/></svg>
            </span>
            <span class="tool-label">混色器</span>
          </button>
        </div>
    ...
    <div class="sub-toolbar">
      ...
      <template v-if="currentMode === 'draw'">
        ...
        <div class="sub-control">
          ...
          <input
            ...
            class="slider"
          />
          <span class="value">{{ brushHardness }}%</span>
        </div>
      ...
      <!-- 污点修复笔刷设置 -->
      <template v-if="currentMode === 'spot-heal'">
        <div class="sub-control">
          <label>大小</label>
          <input
            type="range"
        ...
        <div class="mask-controls">
          <span class="mask-hint">涂抹要修复的污点区域，保存后使用本地 OpenCV 修复</span>
          <button class="clear-mask-btn" @click="clearMaskCanvas(); renderMaskPreview();">
            清除涂抹
          </button>
        </div>
    ...
    <div class="editor-workspace" :class="{ 'with-mixer': currentMode === 'filter' }">
      <!-- 画布区域 -->
      <div class="editor-canvas-container" @wheel.prevent="handleWheel">
        ...
        >
          <div
            ...
            :class="{
              'brush-mode': currentMode === 'draw' || currentMode === 'mask' || currentMode === 'spot-heal' || isLiquifyActive,
      ...
      <aside v-if="currentMode === 'filter'" class="color-mixer-panel">
        <div class="mixer-header">
          <h3>调色混色器</h3>
          <button class="mixer-reset-btn" @click="resetFilters">重置</button>
        </div>

        <div class="mixer-tabs">
          <button
            v-for="tab in mixerTabs"
            :key="tab.id"
            class="mixer-tab"
            :class="{ active: activeMixerTab === tab.id }"
            @click="setMixerTab(tab.id)"
          >
            {{ tab.label }}
          </button>
        </div>

        <div v-if="activeMixerTab !== 'liquify'" class="mixer-controls color-channel-list">
          <div
            v-for="channel in mixerColorChannels"
            :key="channel.id"
            class="mixer-control color-channel-row"
          >
            ...
            <input
              ...
              v-model.number="hslMixer[channel.id][activeMixerTab]"
              class="mixer-slider"
              type="range"
        ...
        <div v-else class="mixer-controls liquify-controls">
          <div class="mixer-control">
            <div class="mixer-control-header">
              <label for="liquify-brush-size">画笔大小</label>
              <span class="mixer-value">{{ liquifyBrushSize }}px</span>
            </div>
            <input
              id="liquify-brush-size"
              v-model.number="liquifyBrushSize"
              class="mixer-slider"
              type="range"
              min="5"
              max="300"
              step="1"
              style="--mixer-track: linear-gradient(90deg, #525252, #f5f5f5)"
            />
          </div>

          <div class="mixer-control">
            <div class="mixer-control-header">
              <label for="liquify-pressure">压力</label>
              <span class="mixer-value">{{ liquifyPressure }}%</span>
            </div>
            <input
              id="liquify-pressure"
              v-model.number="liquifyPressure"
              class="mixer-slider"
              type="range"
              min="0"
              max="100"
              step="1"
              style="--mixer-track: linear-gradient(90deg, #2563eb, #f8fafc)"
            />
          </div>

          <button class="mixer-reset-btn liquify-reset-btn" @click="resetLiquify">
            重置液化
          </button>
        </div>
      </aside>
...
.color-mixer-panel {
  ...
  padding: 18px;
  background: var(--canvas-bg-tertiary, #1a1a1a);
  border-left: 1px solid var(--canvas-border-subtle, #2a2a2a);
  overflow-y: auto;
  box-sizing: border-box;
}
...
.mixer-slider {
  width: 100%;
  height: 6px;
  accent-color: var(--canvas-text-primary, #ffffff);
  cursor: pointer;
}
...
.mixer-slider::-webkit-slider-runnable-track {
  height: 5px;
  border-radius: 999px;
  background: var(--mixer-track, var(--canvas-text-secondary, #a0a0a0));
}
