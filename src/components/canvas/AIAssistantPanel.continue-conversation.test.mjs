...
test('增强模式发送时把历史参考图合并进本轮附件', () => {
  ...
  assert.match(block, /const userAttachments = resolveAssistantAttachmentsForSend/)
...
test('普通模式发送时同样合并历史参考图', () => {
  ...
  assert.equal((panel.match(/const userAttachments = resolveAssistantAttachmentsForSend/g) || []).length, 2)
  // 两处都按 URL 去重合并历史参考图
