    ...
    <section class="voice-picker" role="dialog" aria-modal="true" aria-label="音色选择">
      ...
      <div class="voice-picker-toolbar">
        <div class="voice-picker-tabs" role="tablist" aria-label="音色分类">
          ...
          <button type="button" :class="{ active: activeTab === 'clone' }" @click="activeTab = 'clone'">克隆新音色</button>
        </div>
        ...
        <div class="clone-panel">
          ...
          <div class="clone-recorder">
            ...
            <div v-if="recordedBlob" class="clone-save-form">
              ...
              <button type="button" class="clone-save-button" :disabled="cloneSaving || !cloneName" @click="saveCloneVoice">
      ...
      <div v-else-if="activeTab === 'mine'" class="voice-picker-list">
        ...
        <p v-else-if="!mineVoices.length" class="voice-empty">还没有个人音色，去"克隆新音色"创建一个吧</p>
...
const props = defineProps({ modelValue: { type: Object, default: null }, readingTexts: { type: Array, default: () => [] } })
...
async function loadMineVoices() {
  ...
  mineError.value = ''
  try {
    const response = await apiClient.get('/api/audio/user-voices')
    ...
    mineVoices.value = list.map(item => ({
      ...
      sourceVoice: item.sourceVoice || item.reference_audio_url || '',
...
function getAudioDuration(file) {
  return new Promise((resolve, reject) => {
    ...
    audio.onerror = () => {
      URL.revokeObjectURL(audioUrl)
      reject(new Error('无法读取 MP3 文件时长'))
    }
    audio.src = audioUrl
  })
}
...
async function saveCloneVoice() {
  if (!recordedBlob.value || !cloneName.value) return
