...
test('按秒计费默认不自动选择固定时长', () => {
  ...
  assert.match(source, /isPerSecondBilling\.value && !hasExplicitDurationSelection\.value[\s\S]*selectedDuration\.value = ''/)
...
test('通用按秒计费模型不显示固定时长和主栏计费说明', () => {
  assert.match(source, /v-if="!isPerSecondBilling && !isVeo3Model && !isKlingMotionControl && durations\.length > 0 && durations\.length < 6"/)
  assert.match(source, /v-if="!isPerSecondBilling && !isVeo3Model && !isKlingMotionControl && durations\.length >= 6"/)
