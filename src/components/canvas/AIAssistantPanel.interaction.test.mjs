...
test('queue: force insert deletes queued turn then cancels active turn via shared helper', () => {
  // 取消当前回合统一走 cancelActiveTurnAndWait（等待终态，防 thread-store conflict）
  ...
  // 服务端队列项分支：先删除服务端队列消息，再取消当前回合（共享 cancelActiveTurnAndWait）
  ...
  // 本地乐观项（无 turn_id）：直接从本地队列移除后强插，不调用服务端删除
