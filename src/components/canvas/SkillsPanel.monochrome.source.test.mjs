...
const [assistantSource, marketSource, canvasSource] = await Promise.all([
  ...
  readFile(new URL('./SkillsMarket.vue', import.meta.url), 'utf8'),
  readFile(new URL('../../views/Canvas.vue', import.meta.url), 'utf8')
])

test('Skills market entry stays grayscale while the canvas Skills button keeps its gold mark', () => {
  ...
  assert.match(canvasButtonStyles, /#fff8d6|#f6d77d|#b87a19|#f5c96a|#7b4b0b|#7a4b00/)
})
